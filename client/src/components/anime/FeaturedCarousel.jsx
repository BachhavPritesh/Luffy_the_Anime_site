...
export default function FeaturedCarousel({ items, loading, error }) {
  const [current, setCurrent] = useState(0);
  const [isPaused, setIsPaused] = useState(false);
  const itemsArray = items?.slice(0, 5) || [];

  useEffect(() => {
    if (itemsArray.length < 2 || isPaused) return;
    const interval = setInterval(() => {
      setCurrent((prev) => (prev + 1) % itemsArray.length);
    }, 6000);
    return () => clearInterval(interval);
  }, [itemsArray.length, isPaused]);
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 pt-20">
      <div
        className="relative w-full aspect-[16/9] md:aspect-[21/9] rounded-2xl overflow-hidden shadow-xl isolate"
      ...
      >
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="absolute inset-0 bg-[#0a0a0f] bg-cover bg-center bg-no-repeat"
            style={{
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10 pb-12">
          ...
          >
            <h1 className="font-display text-[clamp(2rem,5vw,3.5rem)] leading-none text-luffy-text mb-4 drop-shadow-lg">
              {anime.title}
            </h1>
            ...
            <p className="text-luffy-muted text-sm md:text-base line-clamp-3 mb-6 max-w-xl">
              {anime.synopsis}
            </p>
